...
| 10. Workload and SPEC hard closure | ❌ Nightly/runtime blocker | Benchmark/PolyBench/portfolio/ctuning artifact publication, PTO kernel parity, and TSVC compile-only PR coverage are green in the PR lane, but SPEC Stage A still depends on hosted shared-musl packaging plus the same late Linux userspace lane, and TSVC QEMU runtime remains separately blocked. |
...
- SPEC Stage A remains a real runtime blocker behind the opt-in PR gate: the lane should now use firmwareless QEMU boot by default, but it is still blocked on both hosted `phase-c` shared-musl packaging (`libc.so` absent) and the same late Linux userspace/rootfs runtime path that currently blocks BusyBox validation.
