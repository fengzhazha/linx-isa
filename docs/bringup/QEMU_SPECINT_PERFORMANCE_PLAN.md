...
2. Add a fast guest-text read path for BSTART checks. `cpu_memory_rw_debug`
   is convenient but expensive because it walks through debug access and full
   address-space translation. For validated executable user mappings, read the
   instruction bytes through the normal TLB/probe path or a small page-local
   decode cache.
