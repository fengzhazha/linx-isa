...
- [x] ID: SPEC-M05-LATEST-TRAIN-ALL-20260628 Latest train-all failure ledger is current.
  Command: `LINX_QEMU_HEARTBEAT_CODE_BYTES=0 SPECINT_TRAIN_ALL_TIMEOUT=180 SPEC_GUEST_HEARTBEAT_SEC=0 SPEC_QEMU_HEARTBEAT_INTERVAL=50000000 SPEC_NO_PROGRESS_TIMEOUT=120 python3 tools/bringup/run_specint_fast_gate.py --profile train --spec-dir workloads/spec2017/cpu2017v118_x64_gcc12_avx2 --qemu emulator/qemu/build-linx/qemu-system-linx64 --sysroot out/libc/musl/install/phase-b --out-dir workloads/generated/specint-train-all-20260628-call-skip-join-r1 --append-extra norandmaps --continue-on-fail`
  Evidence: `workloads/generated/specint-train-all-20260628-call-skip-join-r1/specint_fast_gate_summary.json`, `workloads/generated/specint-train-all-20260628-call-skip-join-r1/train-all/qemu_matrix_summary.json`, and `workloads/generated/specint-train-all-20260628-call-skip-join-r1/train-all/initramfs/stage_b_summary.json`.
  Result: `ok=false`, elapsed `553.933s`; no failed benchmark was classified as `stalled`, and every failed benchmark had `heartbeat_running=true` plus `heartbeat_site_progress=true`, so the current failures are not global QEMU deadlocks.

  | Benchmark | Status | Latest evidence | Proposed owner / next step |
  | --- | --- | --- | --- |
  | `500.perlbench_r` | `user-arithmetic-range` | `Range iterator outside integer range at lib/Math/BigInt.pm line 2675`, count `3350000010`, BPC `0x1555714dde` | Minimize a Linx Perl scalar/range smoke and compare compiler/QEMU scalar IV/NV flag behavior before changing SPEC packaging. |
  | `502.gcc_r` | `user-trap` | `addr=0x10`, user `tpc=0x1556074e88`, user `bpc=0x1556074e80`, count `5650000000`, BPC `0xffffffff801f348a` | EBADF is resolved by the QEMU predicated CALL skip fix. New owner is musl malloc/free-list corruption near static `0x40b1fe88` (`unbin`); run focused malloc/free tracing or PC-watch around `unbin`, `trim`, and `default_malloc`. |
  | `505.mcf_r` | `user-trap` | `addr=0x19`, user `tpc=0x155555b860`, user `bpc=0x155555b85a`, count `1300000005`, BPC `0xffffffff800fb17c` | Use fault regs, call-trace ring, and focused code-byte PC-watch around symbolized simplex/file-state paths; final stdio trap is likely downstream corruption. |
  | `520.omnetpp_r` | `user-trap` | null address trap at `tpc=0xeaea2`, `bpc=0xeae90`, count `750000002`, BPC `0xffffffff803dde02` | Continue as C++ object/callback correctness, not liveness; symbolize the section/config path and inspect static C++ ABI/state. |
  | `523.xalancbmk_r` | `live-timeout` | count `15050000001`, BPC `0xffffffff80048f86`, `stalled=false` | Profile with heartbeat off/coarse, then target BSTART decode/cache and hot helper overhead; also symbolize the recurring kernel breadcrumb. |
  | `525.x264_r` | `kernel-panic` | `LINX_PANIC caller=0xffffffff80001648`, count `450000003`, BPC `0xffffffff803dde02` | Reproduce with a tiny large-initramfs payload, symbolize caller, and inspect early initramfs unpack/page-allocation paths. |
  | `531.deepsjeng_r` | `user-trap` | branch-target trap with `tpc=0`, `bpc=0`, `bpcn=0x1555576390`, count `6700000001`, BPC `0xffffffff800fbe30` | Focus branch-target legality/call-ret state with fault regs, call-trace ring, and PC-watch code bytes around the symbolized user path. |
  | `541.leela_r` | `live-timeout` | count `13800000001`, BPC `0xffffffff803e94a0`, `stalled=false` | Same performance lane as `523`; reduce QEMU overhead after correctness traps are separated. |
  | `557.xz_r` | `user-trap` | bad address `0x04000415794a241f`, user `tpc=0x155557fae8`, user `bpc=0x155557fada`, count `9050000002`, BPC `0xffffffff800fb992` | Treat as pointer/control-flow corruption; symbolize failing block and compare compiler/QEMU branch and memory state. |
  | `999.specrand_ir` | pass | strict hash `0x973dcfc2`, count `450000002`, BPC `0xffffffff803dde02` | Keep as the cheap train pass sentinel for QEMU debug changes. |
...
- [x] ID: SPEC-QEMU-HB-001 BPC heartbeat switch exists.
  ...
  Evidence: `workloads/generated/specint-heartbeat-smoke-20260628/test-smoke/initramfs/999_specrand_ir/run_001/qemu.log`, the current train-all run under `workloads/generated/specint-train-all-20260628-call-skip-join-r1/train-all/initramfs/`, `workloads/generated/specint-heartbeat-regs-smoke-20260628/initramfs/999_specrand_ir/run_001/qemu.log`, and `workloads/generated/specint-heartbeat-code-smoke-20260628-r1/initramfs/999_specrand_ir/run_001/qemu.log`.
  Result: the current train-all run marks no failed benchmark as `stalled`; every failed benchmark has `heartbeat_running=true` and `heartbeat_site_progress=true`. The focused heartbeat-register smoke passes `999.specrand_ir` and emits `LINX_HEARTBEAT_REGS`; the focused heartbeat-code smoke passes `999.specrand_ir` and emits `LINX_HEARTBEAT_CODE`.
...
- [x] ID: SPEC-QEMU-PCWATCH-DBG-001 PC watch can dump full Linx register state.
  Switches: `LINX_DEBUG_PC_WATCH=<pc>[,<pc>...]`, `LINX_DEBUG_PC_WATCH_REGS=1`, `LINX_DEBUG_PC_WATCH_DUMP_CODE_BYTES=<n>`, optional `LINX_DEBUG_PC_WATCH_EXIT=1`, count filters `LINX_DEBUG_PC_WATCH_COUNT_LO=<insns>` / `LINX_DEBUG_PC_WATCH_COUNT_HI=<insns>`, print cap `LINX_DEBUG_PC_WATCH_HIT_LIMIT=<n>`, and shared `LINX_TRACE_REGS=1`.
  Done means: qemu logs emit `linx_pc_watch:` plus optional `LINX_PC_WATCH_REGS` with all GPRs, BPC/TPC context, instruction count at a focused PC, optional `LINX_PC_WATCH_CODE` bytes for symbolizing ambiguous runtime mappings, and `printed=<n>` so noisy symbols can be capped without hiding total hits.
  Evidence: `workloads/generated/specint-pcwatch-regs-smoke-20260628-r1/initramfs/999_specrand_ir/run_001/qemu.log` contains `LINX_PC_WATCH_REGS` for watched PC `0xffffffff80404da6`; `workloads/generated/specint-pcwatch-filter-smoke-20260628-r1/999_specrand_ir/run_001/qemu.log` passes `999.specrand_ir` and contains a filtered `linx_pc_watch:` record with `printed=1 count=655930`.
...
- [ ] ID: SPEC-M05-MALLOC-502 `502.gcc_r` must survive the post-fstat allocator path.
  ...
  Evidence: `workloads/generated/specint-502-after-call-skip-join-20260628-r1/stage_b_summary.json` and `workloads/generated/specint-train-all-20260628-call-skip-join-r1/train-all/initramfs/stage_b_summary.json` classify the benchmark as `user-trap`. With the static PIE slide `0x1515555000`, runtime `0x1556074e88` maps to static `0x40b1fe88` in musl malloc `unbin`, at `c.sdi t#1, [u#1, 16]` with `u#1 == 0`.
  Proposed solution: keep this out of the fd/stat-copyout lane. Add focused malloc/free-list diagnostics around `unbin`, `trim`, and `default_malloc`, or run PC-watch plus `LINX_FAULT_TRACE_REGS=1` on the allocator PCs to identify the earlier heap-list corruption source.
...
- [ ] ID: SPEC-M05-LIVE-SLOW The live slow train workloads need QEMU speedups or longer diagnostic budgets.
  ...
  Evidence:
  - `523.xalancbmk_r`: last heartbeat count `15050000001`, BPC `0xffffffff80048f86`, `progress=site-change`
  - `541.leela_r`: last heartbeat count `13800000001`, BPC `0xffffffff803e94a0`, `progress=site-change`
