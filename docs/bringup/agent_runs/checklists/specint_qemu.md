...
- [x] ID: SPEC-M05-LATEST-TRAIN-ALL-20260628 Latest train-all failure ledger is current.
  ...
  | `502.gcc_r` | `user-trap` | `addr=0x10`, user `tpc=0x1556074e88`, user `bpc=0x1556074e80`, count `5650000003`, BPC `0xffffffff801f346c` | EBADF is resolved by the QEMU predicated CALL skip fix. Current owner is musl oldmalloc free-list corruption near static `0x40b1fe88`; targeted PC-watch captured the bad chunk, so next use `LINX_MEM_TRACE_ADDR=0x1556276048` to find the corrupting store. |
