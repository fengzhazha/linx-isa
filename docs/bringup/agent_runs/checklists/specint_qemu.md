...
- [ ] ID: SPEC-M05-BIGINT-500 `500.perlbench_r` must complete Perl train input.
  ...
  Proposed solution: map the optimized `pp_flop` block around runtime `0x1555829716..0x15558297ae` back to Perl SV flag/value fields and compare the post-helper branch inputs with host behavior. Use selective `pp_ctl.c` compile/probe runs before changing SPEC packaging or QEMU control-flow rules. Keep using BPC heartbeat to distinguish this from QEMU deadlock.
...
- [ ] ID: SPEC-M05-CPP-RUNTIME C++ train workloads must finish after startup.
  ...
  - `523.xalancbmk_r`: stack-edge user trap at `addr=0x3feffffff8`, user `tpc=0x1555a6306a`, `bpc=0x1555a6306a`.
  - `541.leela_r`: stack-edge user trap at `addr=0x3feffffff8`, user `tpc=0x1555623796`, `bpc=0x1555623796`.
  Evidence: `workloads/generated/specint-train-all-20260629-pcwatch-offsets-r1/train-all/initramfs/stage_b_summary.json`, `workloads/generated/specint-cxx-stacklimit-20260628/qemu-focused/qemu_matrix_summary.json`, and the focused register-trace run under `workloads/generated/specint-cxx-after-oldmalloc-20260628/run/initramfs/`.
  Proposed solution: continue `520` as a C++ object/callback correctness trap, and keep `523`/`541` in the finite-stack C++ correctness lane until fault-reg and call-ring traces show whether the `sp=0x3ff0000000` underflow is compiler prologue/epilogue, runtime stack policy, or QEMU memory/branch state.
