...
- [x] ID: SPEC-M05-LATEST-TRAIN-ALL-20260629 Latest train-all failure ledger is current.
  Command: `SPEC_QEMU_HEARTBEAT_INTERVAL=50000000 SPEC_GUEST_HEARTBEAT_SEC=0 SPEC_NO_PROGRESS_TIMEOUT=180 python3 tools/bringup/run_specint_fast_gate.py --profile train --continue-on-fail --qemu emulator/qemu/build-linx/qemu-system-linx64 --sysroot out/libc/musl/install/phase-b --out-dir workloads/generated/specint-train-all-20260629-same-acr-x1-r1 --append-extra norandmaps --heartbeat-sec 30 --qemu-heartbeat-interval 50000000 --guest-heartbeat-sec 0 --no-progress-timeout 180`
  Evidence: `workloads/generated/specint-train-all-20260629-same-acr-x1-r1/specint_fast_gate_summary.json`, `workloads/generated/specint-train-all-20260629-same-acr-x1-r1/train-all/qemu_matrix_summary.json`, and `workloads/generated/specint-train-all-20260629-same-acr-x1-r1/train-all/initramfs/stage_b_summary.json`.
  Result: `ok=false`, elapsed `911.065s`; `999.specrand_ir` passes strict hash. Every failed benchmark has `heartbeat_running=true` and `heartbeat_site_progress=true`, so the current failures are not global QEMU deadlocks. The same-ACR `x1` frame fix removes the prior `500.perlbench_r` `__d_lookup_rcu` Oops and returns 500 to the Perl BigInt range blocker.

  | Benchmark | Status | Latest evidence | Proposed owner / next step |
  | --- | --- | --- | --- |
  | `500.perlbench_r` | `user-arithmetic-range` | `Range iterator outside integer range at lib/Math/BigInt.pm line 2675`; no panic/trap; count `3350000002`, BPC `0x155582976a` | Dlookup Oops is closed. Current owner is Perl scalar/range correctness. |
  ...
  | `505.mcf_r` | `live-timeout` | count `40350000005`, BPC `0x155555c44a`, `stalled=false` | Treat current train input as throughput/live-progress. Reproduce older user traps before moving it back to correctness. |
  | `520.omnetpp_r` | `user-trap` | null trap at `tpc=0xeaea2`, `bpc=0xeae90`, count `750000002`, BPC `0xffffffff800fcede` | Continue as C++ object/callback correctness. Use fault regs plus call trace around the section/config path. |
  | `523.xalancbmk_r` | `user-trap` | trap `addr=0x3feffffff8`, user `tpc=0x1555a6306a`, `bpc=0x1555a6306a`, count `10350000005`, BPC `0xffffffff800fe0da` | Reclassify from live-slow to correctness under effective finite stack. Use fault regs/code bytes around the C++ user trap. |
  ...
  | `531.deepsjeng_r` | `live-timeout` | count `30450000007`, BPC `0x155556764a`, `stalled=false` | Current train input is slow/live. Use a focused profile and compare against the earlier test-input pass profile to find remaining QEMU hot helpers. |
  | `541.leela_r` | `user-trap` | trap `addr=0x3feffffff8`, user `tpc=0x1555623796`, `bpc=0x1555623796`, count `5950000004`, BPC `0xffffffff803dde02` | Group with the C++/stack-edge user traps, not live-slow. |
  | `557.xz_r` | `live-timeout` | count `30500000010`, BPC `0x155558d612`, recent unique sites `4`, `stalled=false` | Current train input is slow/live. Profile before pursuing older bad-pointer evidence. |
  | `999.specrand_ir` | pass | strict hash `0x973dcfc2`, count `450000000`, BPC `0xffffffff803dde02`; focused raw-prlimit smoke `workloads/generated/specint-999-raw-prlimit-20260629-r1/qemu_matrix_summary.json` also passes | Keep as the cheap QEMU debug sentinel. |
...
- [ ] ID: SPEC-M05-BIGINT-500 `500.perlbench_r` must complete Perl train input.
  ...
  Evidence: `workloads/generated/specint-train-all-20260629-same-acr-x1-r1/train-all/initramfs/stage_b_summary.json` classifies `500.perlbench_r` as `user-arithmetic-range` with `Range iterator outside integer range at lib/Math/BigInt.pm line 2675`; heartbeat was still running with site progress, count `3350000002`, and BPC `0x155582976a`. The focused rerun `workloads/generated/specint-500-after-same-acr-x1-20260629-r1/stage_b_summary.json` reproduces the same class without a kernel panic. Older evidence in `workloads/generated/specint-train-all-20260628-qemu-dump-regs-r1/train-all/initramfs/stage_b_summary.json` and `workloads/generated/specint-500-bigint-debug-20260628-r1/stage_b_summary.json` shows the same BigInt failure before the dlookup regression. Host `/usr/bin/perl -I./lib perfect.pl b 3` in the same run directory exits 0 with the expected first three perfect numbers, so argv/input packaging is not the current owner.
...
- [ ] ID: SPEC-M05-LIVE-SLOW The live slow train workloads need QEMU speedups or longer diagnostic budgets.
  ...
  Evidence:
  - `505.mcf_r`: last heartbeat count `40350000005`, BPC `0x155555c44a`, recent unique sites `7`, `progress=site-change`
  - `525.x264_r`: last heartbeat count `20800000000`, BPC `0xffffffff800019bc`, recent unique sites `4`, `progress=site-change`
  - `531.deepsjeng_r`: last heartbeat count `30450000007`, BPC `0x155556764a`, recent unique sites `8`, `progress=site-change`
  - `557.xz_r`: last heartbeat count `30500000010`, BPC `0x155558d612`, recent unique sites `4`, `progress=same-site`
...
- [ ] ID: SPEC-M05-USERTRAP-502-541 Deterministic userspace traps must be split from live-slow work.
  ...
  Evidence: `workloads/generated/specint-train-all-20260629-same-acr-x1-r1/train-all/initramfs/stage_b_summary.json` and per-benchmark qemu logs. Older focused 505 evidence in `workloads/generated/specint-505-final-faultregs-20260628-r1/initramfs/505_mcf_r/run_001/qemu.log` remains historical unless the latest live-timeout run reproduces that trap.
