...
- [x] ID: SPEC-M05-LATEST-TRAIN-ALL-20260629 Latest train-all failure ledger is current.
  ...
  | --- | --- | --- | --- |
  | `500.perlbench_r` | `kernel-panic` | `LINX_DIE msg=Oops` in `__d_lookup_rcu` at `tpc=0xffffffff8013c3de`, followed by `LINX_EXIT_INIT code=0xb`; focused ring-only run `workloads/generated/specint-500-pcwatch-ring-a1-20260629-r1/` records the final watched hit with `x1=0x1`, `x2=0xff6000007dd40027`, `a1=0xff60000004c02d88`, and `traparg0=0x1` | Current owner is dentry/name corruption before the VFS compare loop. Trace the first store to the dentry name pointer field before returning to the older Perl BigInt user-range class. |
...
- [ ] ID: SPEC-M05-DLOOKUP-500 `500.perlbench_r` must stop corrupting the VFS dentry/name lookup path.
  Current blocker: the latest raw-prlimit train-all run and focused reruns panic in kernel `__d_lookup_rcu` at `tpc=0xffffffff8013c3de`, `bpc=0xffffffff8013c3d4`, then exit init with code `0xb`.
  Evidence: `workloads/generated/specint-500-baseline-20260629-r1/stage_b_summary.json` reproduces the Oops; symbolization maps the fault to the `__d_lookup_rcu` byte-compare loop. `workloads/generated/specint-500-dlookup-pcwatch-match-a1-20260629-r1/500_perlbench_r/run_001/qemu.log` captures the active lookup for `a1=0xff60000004c02d88` with compared bytes from the SPEC run path. `workloads/generated/specint-500-pcwatch-ring-a1-20260629-r1/500_perlbench_r/run_001/qemu.log` uses `LINX_DEBUG_PC_WATCH_PRINT=0`, `LINX_DEBUG_PC_WATCH_RING=1`, and `LINX_FAULT_TRACE_PC_LO/HI=0xffffffff8013c3de`; the final ring entry before the Oops records `x1=0x1`, `x2=0xff6000007dd40027`, `a0=0xff60000004c020c0`, `a1=0xff60000004c02d88`, and `traparg0=0x1`.
  Proposed solution: trace the first store to the dentry name pointer field loaded by `ldi [a1, 32], ->x1`. If guest code writes the bad pointer, move ownership to Linux/VFS or libc path mutation; if memory remains valid but QEMU reads `0x1`, reduce to an AVS/QEMU `ldi`/`lbui`/`addi` register-transfer case. Keep BigInt under `SPEC-M05-BIGINT-500` as the next blocker after this kernel path is closed.

- [ ] ID: SPEC-M05-BIGINT-500 `500.perlbench_r` must complete Perl train input.
  Current blocker: this is currently behind `SPEC-M05-DLOOKUP-500`; older focused runs reached Perl user code and failed with `user-arithmetic-range`.
  Evidence: `workloads/generated/specint-train-all-20260628-qemu-dump-regs-r1/train-all/initramfs/stage_b_summary.json` classifies `500.perlbench_r` as `user-arithmetic-range` with `Range iterator outside integer range at lib/Math/BigInt.pm line 2675`; heartbeat was still running with site progress, count `3350000001`, and BPC `0x15556ec224`. The focused rerun `workloads/generated/specint-500-bigint-debug-20260628-r1/stage_b_summary.json` reproduced the same class in 24.454s with site-progress heartbeat and BPC `0x155570bb32`. Host `/usr/bin/perl -I./lib perfect.pl b 3` in the same run directory exits 0 with the expected first three perfect numbers, so argv/input packaging is not the current owner.
...
- [ ] ID: SPEC-M05-LIVE-SLOW The live slow train workloads need QEMU speedups or longer diagnostic budgets.
  ...
  Evidence:
  - `523.xalancbmk_r`: last heartbeat count `27850000004`, BPC `0xffffffff80090c62`, recent unique sites `6`, `progress=site-change`
  - `525.x264_r`: last heartbeat count `24050000000`, BPC `0xffffffff803e8f46`, recent unique sites `4`, `progress=site-change`
  - `541.leela_r`: last heartbeat count `11850000002`, BPC `0xffffffff80090e84`, recent unique sites `8`, `progress=site-change`
  Note: `505.mcf_r` and `557.xz_r` are current user traps, and should be handled under correctness first.
  Proposed solution: profile with heartbeat off or very coarse; target page-local BSTART decode caching, TB chaining, template/queue fast helpers, and removal of helper probes from hot paths. For `523`, first symbolize the recurring `0xffffffff80090c62` kernel loop to distinguish a kernel wait/poll loop from benchmark compute. Current samples are under `workloads/generated/specint-train-all-20260628-heartbeat-stacklimit/profile/`.

- [ ] ID: SPEC-M05-USERTRAP-505-531 `505.mcf_r`, `531.deepsjeng_r`, and `557.xz_r` must stop trapping in userspace.
  Current blockers:
  - `505.mcf_r`: small-address trap with `addr=0x19`, `tpc=0x155555b860`, `bpc=0x155555b85a`, and heartbeat BPC `0xffffffff800fb0ba`.
