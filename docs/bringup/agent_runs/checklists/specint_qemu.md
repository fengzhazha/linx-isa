...
- [x] ID: SPEC-QEMU-PCWATCH-DBG-001 PC watch can dump full Linx register state.
  ...
  Done means: qemu logs emit `linx_pc_watch:` plus optional `LINX_PC_WATCH_REGS` with all GPRs, BPC/TPC context, instruction count at a focused PC, optional `LINX_PC_WATCH_CODE` bytes for symbolizing ambiguous runtime mappings, optional memory words from one or more selected register/queue pointers, and `printed=<n>` so noisy symbols can be capped without hiding total hits.
