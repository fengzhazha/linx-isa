...
- [ ] ID: SPEC-M05-LATEST-TRAIN-ALL-20260628 Latest train-all failure ledger is current.
  Command: `LINX_QEMU_HEARTBEAT_CODE_BYTES=0 SPECINT_TRAIN_ALL_TIMEOUT=180 SPEC_GUEST_HEARTBEAT_SEC=0 SPEC_QEMU_HEARTBEAT_INTERVAL=50000000 SPEC_NO_PROGRESS_TIMEOUT=120 python3 tools/bringup/run_specint_fast_gate.py --profile train --spec-dir workloads/spec2017/cpu2017v118_x64_gcc12_avx2 --qemu emulator/qemu/build-linx/qemu-system-linx64 --sysroot out/libc/musl/install/phase-b --out-dir workloads/generated/specint-train-all-20260628-pcwatch-filter-r1 --append-extra norandmaps --continue-on-fail`
  Evidence: `workloads/generated/specint-train-all-20260628-pcwatch-filter-r1/specint_fast_gate_summary.json`, `workloads/generated/specint-train-all-20260628-pcwatch-filter-r1/train-all/qemu_matrix_summary.json`, and `workloads/generated/specint-train-all-20260628-pcwatch-filter-r1/train-all/initramfs/stage_b_summary.json`.
  Result: `ok=false`, elapsed `546.143s`; no failed benchmark was classified as `stalled`, and every failed benchmark had `heartbeat_running=true` plus `heartbeat_site_progress=true`, so the current failures are not global QEMU deadlocks.

  | Benchmark | Status | Latest evidence | Proposed owner / next step |
  | --- | --- | --- | --- |
  | `500.perlbench_r` | `user-arithmetic-range` | `Range iterator outside integer range at lib/Math/BigInt.pm line 2675`, count `3350000000`, BPC `0x15556fda18` | Minimize a Linx Perl scalar/range smoke and compare compiler/QEMU scalar IV/NV flag behavior before changing SPEC packaging. |
  | `502.gcc_r` | `fd-io-bad-file-number` | `fatal error: 200.c: Bad file number`, count `5600000005`, BPC `0xffffffff803dde02` | Instrument GCC `open_file`/errno state; syscall copyout evidence already shows `openat`, `fstat`, and `fcntl` succeed without `-EBADF`. |
  | `505.mcf_r` | `user-trap` | `addr=0x19`, user `tpc=0x155555b860`, user `bpc=0x155555b85a` | Use fault regs, call-trace ring, and focused code-byte PC-watch around symbolized simplex/file-state paths; final stdio trap is likely downstream corruption. |
  | `520.omnetpp_r` | `user-trap` | null address trap at `tpc=0xeaea2`, `bpc=0xeae90` | Continue as C++ object/callback correctness, not liveness; symbolize the section/config path and inspect static C++ ABI/state. |
  | `523.xalancbmk_r` | `live-timeout` | count `16800000000`, BPC `0xffffffff80048d6e`, `stalled=false` | Profile with heartbeat off/coarse, then target BSTART decode/cache and hot helper overhead; also symbolize the recurring kernel breadcrumb. |
  | `525.x264_r` | `kernel-panic` | `LINX_PANIC caller=0xffffffff80001648`, count `450000005` | Reproduce with a tiny large-initramfs payload, symbolize caller, and inspect early initramfs unpack/page-allocation paths. |
  | `531.deepsjeng_r` | `user-trap` | branch-target trap with `tpc=0`, `bpc=0`, `bpcn=0x1555576390` | Focus branch-target legality/call-ret state with fault regs, call-trace ring, and PC-watch code bytes around the symbolized user path. |
  | `541.leela_r` | `live-timeout` | count `13850000005`, BPC `0xffffffff80048f72`, `stalled=false` | Same performance lane as `523`; reduce QEMU overhead after correctness traps are separated. |
  | `557.xz_r` | `user-trap` | bad address `0x04000415794a241f`, user `tpc=0x155557fae8`, user `bpc=0x155557fada` | Treat as pointer/control-flow corruption; symbolize failing block and compare compiler/QEMU branch and memory state. |
  | `999.specrand_ir` | pass | strict hash `0x973dcfc2`, count `450000001`, BPC `0xffffffff803dde02` | Keep as the cheap train pass sentinel for QEMU debug changes. |
...
- [ ] ID: SPEC-M05-PANIC-525 `525.x264_r` must boot far enough to execute userspace.
  Current blocker: early `LINX_PANIC`; the latest train-all log reports caller `0xffffffff800016`, while earlier liveness/heartbeat runs reported `0xffffffff80001648` or a truncated `0xffffffff80001`.
  Evidence: `workloads/generated/specint-train-all-20260628-heartbeat-code-r1/train-all/initramfs/525_x264_r/run_001/qemu.log`, `workloads/generated/specint-train-all-20260628-liveness-v2/train-all/initramfs/525_x264_r/run_001/qemu.log`, `workloads/generated/specint-train-all-20260628-heartbeat-stacklimit/run/initramfs/525_x264_r/run_001/qemu.log`, and the older `workloads/generated/specint-train-all-20260628-after-oldmalloc/train-all/initramfs/525_x264_r/run_001/qemu.log`.
