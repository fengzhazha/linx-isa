== Tile / Vector Block Instructions (v0.56)

This chapter defines the **normative header contracts** for tile/vector/tensor-style block types in the live v0.56
profile.
...
* the opcode catalog notes (e.g. `isa/v0.56/opcodes/lx_32.opc`).
...
* `B.IOT` / `B.IOT` — tile binding and tile size (strict range `0B..512KB`)
...
=== Rendering profile (v0.56 canonical)

The first frozen rendering-oriented v0.56 profile uses the following tile conventions:
...
* `B.IOT` / `B.IOT` is **mandatory** (tile binding + tile size).
...
* `B.IOT` / `B.IOT` is **mandatory** (bind source/destination tiles and tile sizes).
...
* `B.IOT` / `B.IOT` is **mandatory** (bind operand tiles and sizes).
...
* `B.DIM` is **mandatory**.
* `B.IOT` / `B.IOT` is **mandatory** (bind destination tile + size).
