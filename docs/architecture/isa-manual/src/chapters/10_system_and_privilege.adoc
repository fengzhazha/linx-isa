[[system]]
== System and privileged architecture (v0.56)

This chapter defines the v0.56 privileged architecture: Access Control Rings (ACR), privileged system registers
...
Bring-up conventions (v0.56 current profile):
...
Informative software role mapping (v0.56 current profile):
...
+
+* Template replay flags (bring-up): implementations SHOULD expose a single EBARG register `EBARG_TPLFLAGS_ACRm` with template trap/replay flags. The bit layout is defined in <<ssr-ebarg>> (see `EBARG_TPLFLAGS_ACRn`).
...
+|===
+
+NOTE: earlier draft text previously used `E_INST(EC_ILLEGAL)` and `E_INST(EC_PARAM)` informally without allocating numeric encodings. Canonical `v0.56` allocates concrete `EC_*` values above.
+
...
|`E_INST` (illegal instruction / illegal SSR) |`ILLEGAL_INST` (4) |ACR0 |ACR1 |ACR1
|`E_DATA` (data access) |`DATA_ALIGN_FAULT` / `DATA_PAGE_FAULT` (34/35) |ACR0 |ACR1 |ACR1
|`E_BLOCK` (block-format / CFI / body-fetch) |`BLOCK_TRAP` (5) |ACR0 |ACR1 |ACR1
|`E_ASSERT` (assertion failure) |`ASSERT_FAIL` (52) |ACR0 |ACR1 |ACR1
|`E_BREAKPOINT` |`SW_BREAKPOINT` / `HW_BREAKPOINT` (50/49) |ACR0 |ACR1 |ACR1
|`E_ISSR` (illegal SSR access) |`ILLEGAL_INST` (4) |ACR0 |ACR1 |ACR1
|===
...
[NOTE]
====
These items remain platform-specific policy inputs on top of the canonical v0.56 privileged architecture.
