...
[[memory-model]]
=== Memory model and ordering (v0.56 current)
...
* Store→store order is preserved for all addresses (including `TSTORE` beats).
...
[[memory-channels]]
=== BCC and MTC channels (v0.56 current)

v0.56 explicitly models two architectural memory issue channels inside one TSO ordering domain:

* **BCC channel**: scalar `load/store/atomic/fence` operations and ordering point for scalar blocks.
* **MTC channel**: tile-oriented memory blocks (`TLOAD`, `TSTORE`) and bridged shader-kernel memory issue
  (`l.*.brg`, `v.*.brg`) through `MPAR`/`MSEQ`.

Legacy naming note: historical `BSTART.MCALL` text maps to canonical `BSTART.MPAR`/`BSTART.MSEQ`.
Legacy raw `TPREFETCH` text is archived only and is not a live v0.56 encoded operation.
...
* Entering a canonical `MPAR`/`MSEQ` shader kernel requires an acquire-style boundary before kernel-side memory issue begins.
* Exiting a canonical `MPAR`/`MSEQ` shader kernel requires a release-style boundary before subsequent scalar blocks execute.
* While a shader kernel is active, BCC scalar memory issue is architecturally closed for correctness.

[[memory-tload-tstore]]
=== TLOAD/TSTORE semantics

`TLOAD` and `TSTORE` are template-style tile memory operations. The canonical architectural model is:
...
* Address footprint is 2D and derived from descriptor state:
  ...
  - tile IO bindings and transfer size from one-or-more `B.IOT`
  - element data type from `BSTART.TMA`

Layout and dimension contract (v0.56 current profile for TMA):
...
* Conceptual GM-side bounding rectangle (reconciled to the v0.56 descriptor model):
  - `Base` is the bound base-pointer argument from `B.IOR`.
  - `Stride` is the bound row/outer-stride argument from `B.IOR`.
  - `ElemBytes` is derived from the `BSTART.TMA` data type.
  - top-left corner: `Base`
  - top-right exclusive corner: `Base + LB0 * ElemBytes`
  - bottom-left exclusive corner: `Base + LB1 * Stride`
  - bottom-right exclusive corner: `Base + LB0 * ElemBytes + LB1 * Stride`
* TR-side outer element count is derived as:
  - `TR_outer = TileElements / LB2`
  - `TileElements = TileBytes / element_size`, where `TileBytes` comes from the active `B.IOT`/size descriptor.
...
* The effective base pointer register is taken from the most recent `B.IOR` in the header:
  ...
  - otherwise fall back to `RegSrc0` (base-only binding).
...
* Internal beats of one operation are not guaranteed to issue in source-program order; software must not assume row-major,
  element-major, or beat-commit order within one architectural `TLOAD`/`TSTORE`.
* Unaligned shared-memory access is allowed unless tightened by platform policy.
* Raw material that contradicts group-overlap semantics from archived sources is not part of canonical v0.56 behavior; selector/group
  space not defined by the catalog is reserved/illegal.

Canonical v0.56 profile constraints:

* `B.ARG + B.IOR` are mandatory in descriptor-rich `TLOAD/TSTORE` headers.
* TMA execution state is not sourced from `B.CATR/B.DATR`; `TLOAD/TSTORE` consume only `BSTART/B.ARG/B.DIM/B.IOR/B.IOT`.
* Repeated `B.IOR` and repeated `B.IOT` are legal and are consumed in header order.
* `B.IOT` dynamic-size descriptors are legal only when `RegSrc` resolves to strict `0B..512KB`; runtime rejects invalid
  dynamic sizes.
* For bring-up code size, emit `C.B.DIMI` for `LB0/LB1/LB2` when the value fits in `imm8` (and `RegSrc=zero`).
* Raw `group=2` material and broad fixed-pipe/TCVT metadata flows are reserved/illegal in canonical v0.56 until a future
  catalog revision assigns explicit semantics.

Preferred v0.56 textual form for tile memory blocks is:
...
For completeness: v0.56 also permits non-memory typed TMA headers `BSTART.TMOV <DataType>`
(encoding: `BSTART.TMA Function=2`), but those are governed by <<blockisa-tile>> (not by memory-operation semantics).

`B.IOT` descriptor rendering uses `last` for the terminal descriptor and prints transfer footprints in KiB (for example
`->t<1KB>`, `->acc<4KB>`). The strict architectural range is `0B..512KB`.

[[memory-mcall]]
=== Shader-kernel boundary semantics (`MPAR` / `MSEQ`)

Canonical shader-kernel behavior is expressed through `.MPAR` / `.MSEQ` execution modes (`MCALL` is a historical alias):

* Execution is non-speculative until declared input dependencies (`B.IOT`/`B.IOT`) are resolved.
...
* Archived VREG-only raw local-register fragments are not part of canonical v0.56 behavior.

Bridged address-formation contract (canonical v0.56):
...
* `l.lw.brg ->t, [ri0, 16]`
** one scalar-uniform bridged access using `ri0 + 16`.
* `v.sw.brg vt#1, [ri0, lc0<<2, lc1<<10]`
** per-lane bridged stores where the base rule is still explicit-only:
** `ri0 + (lc0 << 2) + (lc1 << 10)`
** no additional implicit `lc0` term is injected by the `.brg` domain itself.

[[memory-types]]
==== Memory types (Normal vs Device)

Linx platforms classify memory addresses into types. This v0.56 manual uses two
high-level types:
...
The v0.56 Linx toolchain and emulator model are **little-endian**: the least-significant byte of a multi-byte value is
...
The catalog includes `*.PCR` forms that compute an address relative to the current PC/TPC (as defined by the v0.56 catalog).
These forms are used by toolchains for efficient access to nearby constants and for position-independent code patterns.

[[memory-atomics]]
=== Atomics (overview)

The v0.56 catalog includes load-reserved/store-conditional (`LR.*`/`SC.*`) and read-modify-write style atomic operations
with qualifier suffixes (for example `.aq`, `.rl`). The precise memory ordering semantics are defined by
`isa/v0.56/state/memory_model.json` and the architectural model implemented by the toolchain/QEMU/RTL.
...
The v0.56 catalog defines two fence instructions:
...
Bring-up definition (v0.56 current profile):
...
Semantics (v0.56 current profile):
