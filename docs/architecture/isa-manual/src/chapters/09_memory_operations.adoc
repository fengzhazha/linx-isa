...
[[memory-model]]
=== Memory model and ordering (v0.4 current)
...
[[memory-channels]]
=== BCC and MTC channels (v0.4 current)

v0.4 explicitly models two architectural memory issue channels inside one TSO ordering domain:
...
Legacy raw `TPREFETCH` text is archived only and is not a live v0.4 encoded operation.
...
Layout and dimension contract (v0.4 current profile for TMA):
...
* Conceptual GM-side bounding rectangle (legacy raw description reconciled to the v0.4 descriptor model):
...
* Contradictory raw group-overlap fragments from archived material are not part of canonical v0.4 behavior; selector/group
  space not defined by the catalog is reserved/illegal.

Canonical v0.4 profile constraints:
...
* Raw `group=2` material and broad fixed-pipe/TCVT metadata flows are reserved/illegal in canonical v0.4 until a future
  catalog revision assigns explicit semantics.

Preferred v0.4 textual form for tile memory blocks is:
...
For completeness: v0.4 also permits non-memory typed TMA headers `BSTART.TMOV <DataType>`
...
Linx platforms classify memory addresses into types. This v0.4 manual uses two
high-level types:
...
The v0.4 Linx toolchain and emulator model are **little-endian**: the least-significant byte of a multi-byte value is
...
The v0.4 catalog includes load-reserved/store-conditional (`LR.*`/`SC.*`) and read-modify-write style atomic operations
with qualifier suffixes (for example `.aq`, `.rl`). The precise memory ordering semantics are defined by
`isa/v0.4/state/memory_model.json` and the architectural model implemented by the toolchain/QEMU/RTL.
...
The v0.4 catalog defines two fence instructions:
...
Bring-up definition (v0.4 current profile):
...
Semantics (v0.4 current profile):
