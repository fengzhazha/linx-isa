...
Address translation (v0.4 current profile, preserving the v0.2 MMU baseline):
...
* Legacy trap-save singleton naming from pre-v0.2 drafts is not part of canonical v0.4 artifacts.
...
NOTE: The v0.4 `reg5` table also includes special spellings used by the assembler/disassembler for queue operations
...
=== Shader-kernel lane policy (v0.4 current)

For canonical shader-kernel execution (`BSTART.MPAR` / `BSTART.MSEQ`), v0.4 locks an explicit inactive-lane policy:
...
[[progmodel-vpred]]
=== Block-control predicate and kernel EXEC mask (v0.4)
...
* Canonical shader-kernel special operands are:
  ...
  - `p` and `zero`.
* Archived raw operand names `TE..TH` and `TO1/TO2/TO3` are not assigned meaning in canonical v0.4 and remain
  reserved/unassigned.
...
* Direct `v.*` writes into scalar/group-domain destinations remain illegal unless the instruction is an explicitly
  ...
  Example:
** `v.add vt#1, ri0, ->t` is illegal in canonical v0.4.
** `v.rdadd vt#1, ->t` is legal because `V.RD*` is an explicit reduction family.

SIMT group model (v0.4 current profile):
...
* Canonical compilers must preserve the meanings `LB0 = lanes per group` and `LB1 = group count`; `LB0` is not reinterpreted
  as total logical threads in canonical `v0.4`.
...
Bring-up conventions (v0.4 current profile, with the v0.2 trap ABI baseline):
...
named **ACR0..ACR15**, which are organized as a tree rooted at **ACR0** (see <<system-acr-tree>>). In the v0.4 current
profile:
...
NOTE: In the v0.4 current profile (same as the v0.2 baseline), floating-point exceptions do **not** trap; they only update sticky flags in `FCSR`
