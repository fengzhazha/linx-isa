...
Many arithmetic and load instructions have destinations described as `->{t, u, Rd}` in the v0.56 catalog, indicating:
...
The v0.56 ISA tables use several common conventions:

* `.sw` / `.uw` suffixes annotate sign/zero extension or “use as signed/unsigned word” in mixed-width operations.
* Some immediate forms (notably `SETC.*I`) use a **split immediate** encoding (`simm/uimm` plus an encoded `shamt`). Assembly syntax prints a single immediate; architecturally the value is interpreted as `imm = (SignExtend(simm) << shamt)` (or `ZeroExtend(uimm) << shamt`).
* Bracketed memory operands take the form `[...]` with base + offset, or base + index with optional shift.
* In canonical v0.56 shader kernels, bridged global-memory forms spell the access domain explicitly: `l.*.brg` or `v.*.brg`.

Refer to `instructions[].asm` in `isa/v0.56/linxisa-v0.56.json` for the full syntax used by each instruction form.
