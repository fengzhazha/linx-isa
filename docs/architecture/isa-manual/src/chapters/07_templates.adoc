...
NOTE: For `TemplateKind.class=1` (TEPL templates), identification uses the tile opcode selector (`TileOpcode`), not this enum.
...
Additional macro blocks appear in the v0.56 catalog for future use (examples):
...
golden sources (`isa/v0.56/`) and the canonical catalog (`isa/v0.56/linxisa-v0.56.json`).
