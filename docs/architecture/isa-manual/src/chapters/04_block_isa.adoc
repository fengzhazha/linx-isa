...
* Source-level direct calls should use the fused form `BSTART.STD CALL, <target>, ra=<return_label>`.
* Lowered object code may materialize the return address in `ra` using adjacent `addpc/hl.addpc` or `setret/c.setret`
  patterns.
...
after** the call-type block start marker (e.g. `BSTART.STD CALL, <target>` / `C.BSTART CALL`). Tooling and hardware may
