...
* Canonical v0.4 assembly uses typed `BSTART.<type>` forms only; legacy PAR-style shorthand is compatibility input only
...
The v0.4 current profile (preserving v0.2 baseline encodings) defines **three block forms**. Toolchains and implementations MUST enforce the legality
...
Block Split forms. `TMA`, `CUBE`, and `TEPL` selector/descriptor semantics are defined by the v0.4 catalog; selector
...
Bring-up contract (v0.4 current profile):
...
* `TVEC` is not a standalone v0.4 header mnemonic; TVEC-style execution is represented by `BSTART.VPAR`/`BSTART.VSEQ`.
* Historical `MCALL` terminology is represented by `.MPAR`/`.MSEQ` execution modes in the live profile.
* Historical `TPREFETCH` text from raw Janus materials is archived legacy only; no canonical `v0.4` header/selector is assigned.
...
Bring-up v0.4 descriptor rules (normative for this bring-up profile):
...
* `B.IOT` (dynamic-size form) uses `RegSrc` to carry runtime output size. v0.4 accepts only values that decode
...
* `S0V` / `S1V` semantics are *inverted*: `0` means the corresponding `SrcTile*` is present as an input; `1` means absent.
  When a source is absent, its corresponding `SrcTile*` field is ignored and SHOULD be encoded as `0` in canonical v0.4 output.
...
** `4`: implicit accumulator (`acc`)
  (`acc`) is a singleton architectural accumulator in v0.4 (no encoded ACC depth/index).
...
* Each `B.IOT` / `B.IOTI` descriptor carries up to two source-tile references plus one destination tile kind.
...
* Archived raw material that relies on other `group` values is not part of canonical v0.4 and remains reserved/illegal.
* `B.IOR` disassembly omits zero GPR inputs/outputs and prints the input list in encoding order (`RegSrc0`, `RegSrc1`, `RegSrc2`)
  (for example, `[s0,a6],[]`).

v0.4 tile header encoding assignments (from `isa/v0.4/opcodes/lx_32.opc`):
...
v0.4 tile `DataType` field assignments (u5):
...
* Unassigned values are reserved in v0.4 and MUST NOT appear in canonical streams.

v0.4 alias policy (normative):

* Canonical encoded headers are `BSTART.TMA`, `BSTART.CUBE`, `BSTART.TEPL`, `BSTART.VPAR`, `BSTART.VSEQ`.
* Alias mnemonics are explicit v0.4 instruction forms in the golden catalog and resolve to encoding selectors:
...
* Preferred v0.4 textual spellings in docs/tests are the alias mnemonics (`BSTART.TLOAD`, `BSTART.TSTORE`,
...
* Legacy mixed forms such as `BSTART.TMA TLOAD, <DataType>` or `BSTART.CUBE MAMULB, <DataType>` are compatibility input
  only and MUST NOT be used in new v0.4 examples.
* Legacy `PAR` block-start spelling is compatibility input only and MUST NOT appear in canonical v0.4 output streams.

v0.4 TMA `Function` freeze:

* `Function=0` => `TLOAD`
* `Function=1` => `TSTORE`
* `Function=2` => `TMOV`
* `Function=3..31` are reserved in v0.4 and MUST NOT appear in canonical outputs.

v0.4 CUBE `Function` assignment:

* `Function=0` => `TMATMUL` (`MAMULB` compatibility spelling)
* `Function=2` => `TMATMUL.ACC` (`MAMULB.ACC` compatibility spelling)
* `Function=8` => `ACCCVT`

v0.4 TEPL tile opcode allocation policy (current canonical state):

* The authoritative machine-readable selector map is `isa/v0.4/state/engine_ops.json`.
* Standardized v0.4 TEPL assignments use a packed low-6-bit selector inside the 10-bit `TileOpcode` field:
** `TileOpcode[9:6]` MUST be zero in canonical v0.4 streams.
...
** `Mode=1, Function=11..31` (`TileOpcode=0x02B..0x03F`) are reserved in canonical v0.4.
...
* Pre-packed legacy selector assignments from earlier drafts and downstream experiments (for example `TROWMAX=0x020`,
  `TEXP=0x040`, `TGATHER=0x060`, `TCOLEXPAND=0x0C0`) are archived-only and MUST NOT appear in canonical v0.4 outputs.
* Unassigned tile opcode values are reserved/illegal in canonical v0.4; canonical toolchains MUST reject them and
...
* Explicit destination depth encoding (`->t#k`/`->u#k`/`->m#k`/`->n#k`) is not part of v0.4.
...
v0.4 block split patterns and per-family constraints:
...
** no explicit accumulator tile id is encoded in v0.4 descriptors.
...
** v0.4 descriptors bind only destination tile metadata (`->t/u/m/n<size>`); no accumulator source id is encoded.
** is header-driven (no SIMT body; do not use `B.TEXT`).
* `BSTART.TEPL`:
** operation is selected only by the tile opcode (`TileOpcode`).
** standardized v0.4 TEPL assignments use packed `Mode/Function` in `TileOpcode[5:0]`; `TileOpcode[9:6]` MUST be zero.
...
** PTO template-op forms (`TADD`, `TSUB`, `TROWMAX`, etc.) are header-driven in v0.4 (no SIMT body).
* `BSTART.VPAR` / `BSTART.VSEQ`:
** are the only TVEC-equivalent headers.
** require one `B.TEXT` target for the SIMT body in decoupled form.
** no `BSTART.TVEC` encoding exists in v0.4.
...
* TEPL uses a 10-bit tile opcode selector (`TileOpcode`, 0..1023), but canonical v0.4 currently standardizes only the
...
** `TileOpcode[9:6] != 0` remains reserved in canonical v0.4.
...
The v0.4 catalog includes the following descriptor families:
...
* **Block I/O binding**
  ...
  - `B.IOD`: obsolete compatibility descriptor in canonical v0.4 streams.
* **Block arguments**
...
* Tile-byte legality for tile blocks is fail-fast with the compiler/runtime formula:
  ...
  `element_bits` is derived from the v0.4 `DataType` mapping (including packed 4-bit formats).
...
[[blockisa-v03-ai]]
=== v0.4 AI extension rules

The canonical v0.4 profile integrates only reconciled and internally consistent AI-oriented behavior:

* Canonical mnemonic policy: vector canonical forms are `V.*`; legacy raw `L.*` spellings are normalization-only input.
* Canonical block-start policy: use typed `BSTART.*` spellings in v0.4-facing disassembly/docs/tests.
* `B.IOT/B.IOTI` group scheduling is defined for groups `0/1`; contradictory raw `group=2` material is reserved/illegal
  in canonical v0.4.
* `MAMULBMX*` raw fragments with contradictory `group=2` contracts are archived history only and are not part of
  canonical v0.4 outputs.
...
=== v0.4 canonical PTO-to-TAU mapping (auto-mode profile)

The canonical v0.4 auto-mode profile maps PTO tile primitives to typed TAU block headers and descriptors:
...
* `PTO vec template ops` (`TADD`, `TSUB`, `TROWMAX`, ...):
  `BSTART.TEPL(<TileOpcode>)` + descriptor stream (`B.ARG`/`B.IOR`/`B.IOT/B.IOTI`) as required by the op profile.
...
* Canonical v0.4 artifacts MUST NOT encode reserved `group=2` behavior.
...
`TVEC` is not a v0.4 header mnemonic. TVEC-style execution is represented by `BSTART.VPAR`/`BSTART.VSEQ` with
an out-of-line SIMT body selected by `B.TEXT`.

Execution model (normative for v0.4):
...
* Divergent control flow is represented by explicit updates to `p` inside the body; canonical `v0.4` does not require a hidden
  per-lane architectural PC.

Decoupled body form (v0.4 canonical constraint):
...
* Archived raw names `TE..TH` and `TO1/TO2/TO3` are not part of canonical v0.4 and remain reserved/unassigned.
...
** `TO/TS` map to the first two destination tiles encoded in an *absent* source slot (preferring `SrcTile1`, then
   `SrcTile0`), matching the canonical disassembly contract for `B.IOTI`.
...
* Divergent control flow narrows, restores, or otherwise updates `p` under the current group PC; canonical `v0.4` does not rely on
