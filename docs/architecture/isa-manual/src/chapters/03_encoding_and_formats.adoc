...
The following examples illustrate the encoding bitfield layout for representative instructions. These diagrams are automatically generated from the machine-readable catalog.
...
Vector 64-bit (prefix + postfix):

image::../generated/encodings/enc_v_add_part1.svg[V.ADD encoding,width=800]

[NOTE]
====
A complete set of encoding diagrams for all instructions is available in the `generated/encodings/` directory. These diagrams use color-coded fields to distinguish operand types:

* **Blue**: opcode / funct
* **Green**: destination register (rd)
* **Orange**: source registers (rs1, rs2, rs3)
