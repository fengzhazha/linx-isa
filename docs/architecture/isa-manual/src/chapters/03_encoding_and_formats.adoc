...
The v0.56 encoding model defines:
...
Mnemonic prefixes are used to distinguish families of encodings:
...
Within canonical `MPAR`/`MSEQ` shader-kernel bodies, v0.56 adds a unified `lx64` execution rule:

* if any operand names `vt`/`vu`/`vm`/`vn`, the instruction executes in the per-lane `v.*` domain,
* otherwise it executes in the scalar-uniform `l.*` domain,
* scalar/group-domain source operands broadcast to all active lanes when consumed by `v.*`,
* writes back into scalar/group-domain destinations are illegal unless explicitly defined by a reduction or mask-producing form.
...
The authoritative encoding details live in `isa/v0.56/linxisa-v0.56.json`:
...
The generated codec tables in `isa/generated/codecs/` are derived directly from the JSON and are the recommended entry point for
tooling.

[[encoding-diagrams]]
=== Bitfield diagrams

The following examples illustrate the encoding bitfield layout for representative instructions.
These diagrams are automatically generated from the machine-readable catalog and are color-coded
by field type.

32-bit R-type (register-register):

image::../generated/encodings/enc_add.svg[ADD encoding,width=800]

32-bit I-type (immediate):

image::../generated/encodings/enc_addi.svg[ADDI encoding,width=800]

16-bit compressed (C.ADD):

image::../generated/encodings/enc_c_add.svg[C.ADD encoding,width=600]

Vector 64-bit (prefix + postfix, both parts stacked):

image::../generated/encodings/enc_v_add_parts.svg[V.ADD encoding,width=800]

[NOTE]
====
A complete set of encoding diagrams for all 740+ instruction forms is available in the
`generated/encodings/` directory.  The diagrams use a consistent color scheme:

[cols="2,1,3",options="header"]
!===
!Color !Field type !Examples
!#2E7D32 (green) !Destination register !Rd, RegDst
!#1565C0 (blue) !First source register !Rs1, SrcL
!#00796B (teal) !Second source register !Rs2, SrcR
!#E65100 (orange) !Third source / FMA operand !Rs3, SrcD, SrcA
!#6A1B9A (purple) !Opcode / funct / rounding mode !funct3, funct7, opcode, RST_Type
!#AD1457 (pink) !Shift amount !shamt
!#C62828 (red) !Immediate / offset !uimm12, simm12, offset, pred, succ
!#90A4AE (gray) !Zeroed / reserved constant !(0)
!===
====
