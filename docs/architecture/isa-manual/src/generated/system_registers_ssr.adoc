...
// Source: linxisa-v0.56.json (state.system_registers)
[[ssr-table]]
...
|`0x0050` |`BLOCKNUM` |RO |Block Number. Configured by system controller before kernel launch; identifies total logical blocks participating in a parallel task.
|`0x0051` |`BLOCKID` |RO |Block ID. Configured by system controller before kernel launch; identifies the current logical block for debug, trace, and block-level parallel programming.
...
|`0xnf30` |`XBINFO_ACRn` |RW |Cross-block info register. Provides the CAC_TABLE base address for the selected ACR in the v0.56 bring-up profile.
|`0xnf31` |`ACR_PARAM_ACRn` |RW |ACR parameter register. Bring-up uses the low bits to advertise EBSTATE capacity and related context limits.
|===
...
|`0xnf51` |`EBARG_TPLFLAGS_ACRn` |RW |Template replay flags: bit0 TPL_VALID, bit1 TPL_ID_PRESENT, bits11:2 TPL_ID, bits13:12 TPL_PHASE, bit14 TPL_DIRTY, bit15 TPL_REDO_OK, bit16 TPL_RESUME_OK; all higher bits reserved zero.
|===

[[ssr-debug]]
==== Debug SSRs (v0.56 bring-up)
