...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="Directory containing per-test output folders (default: avs/compiler/linx-llvm/tests/out).",
    )
    ap.add_argument(
        "--spec",
        type=Path,
        default=Path(__file__).resolve().parents[4] / "isa/v0.56/linxisa-v0.56.json",
        help="Path to linxisa-v0.56.json (default: isa/v0.56/linxisa-v0.56.json).",
    )
    ap.add_argument(
        "--llvm-project",
        type=Path,
    ...
    if not args.out_dir.exists():
        ...
        print(
            "hint: run ./avs/compiler/linx-llvm/tests/run.sh first (or set --out-dir)",
            file=sys.stderr,
        )
        return 2
