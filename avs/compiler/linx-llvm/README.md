...
- Every MachineBasicBlock starts with a block marker:
  ...
  - `BSTART CALL, <target>, ra=<return-label>` for returning direct calls
...
The compile tests live in `avs/compiler/linx-llvm/tests` and generate:
...
- In particular, fused `ra=` source syntax is the preferred scalar direct-call
  surface, but handwritten `ICALL ra=` is not yet portable on the current
  branch.
