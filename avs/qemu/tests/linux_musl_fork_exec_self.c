...
int main(int argc, char **argv)
{
	...
	if (argc > 1 && strcmp(argv[1], "child") == 0) {
		emit_marker("MUSL_FORK_EXEC_SELF_CHILD");
		_exit(37);
	}

	emit_marker("MUSL_FORK_EXEC_SELF_START");
	pid = raw_fork();
	if (pid < 0) {
		emit_marker("MUSL_FORK_EXEC_SELF_FORK_FAIL");
		poweroff_now();
		return 1;
	}

	if (pid == 0) {
		char *child_argv[] = { "/init", "child", NULL };
		char *child_envp[] = { NULL };

		emit_marker("MUSL_FORK_EXEC_SELF_BEFORE_EXEC");
		{
			...
			int n = snprintf(buf, sizeof(buf),
					 "MUSL_FORK_EXEC_SELF_EXEC_FAIL rc=%d errno=%d",
					 rc, saved_errno);
			if (n > 0)
				emit_marker(buf);
		}
	...
	emit_marker("MUSL_FORK_EXEC_SELF_PARENT");
	waited = waitpid(pid, &status, 0);
	if (waited != pid || !WIFEXITED(status) || WEXITSTATUS(status) != 37) {
		char buf[192];
		int n = snprintf(buf, sizeof(buf),
				 "MUSL_FORK_EXEC_SELF_FAIL pid=%ld waited=%ld status=%d errno=%d",
	...
	emit_marker("MUSL_FORK_EXEC_SELF_PASS");
	poweroff_now();
	return 0;
}
