...
int main(void)
{
	...
	emit_marker("MUSL_FORK_WAIT_RAW_EXIT_START");
	pid = raw_fork();
	if (pid < 0) {
		emit_marker("MUSL_FORK_WAIT_RAW_EXIT_FORK_FAIL");
		poweroff_now();
		return 1;
	}

	if (pid == 0) {
		emit_marker("MUSL_FORK_WAIT_RAW_EXIT_CHILD");
		(void)syscall(SYS_exit, 37);
		emit_marker("MUSL_FORK_WAIT_RAW_EXIT_RETURNED");
		poweroff_now();
		return 3;
	}

	emit_marker("MUSL_FORK_WAIT_RAW_EXIT_PARENT");
	waited = waitpid(pid, &status, 0);
	if (waited != pid || !WIFEXITED(status) || WEXITSTATUS(status) != 37) {
		char buf[192];
		int n = snprintf(buf, sizeof(buf),
				 "MUSL_FORK_WAIT_RAW_EXIT_FAIL pid=%ld waited=%ld status=%d errno=%d",
	...
	emit_marker("MUSL_FORK_WAIT_RAW_EXIT_PASS");
	poweroff_now();
	return 0;
}
